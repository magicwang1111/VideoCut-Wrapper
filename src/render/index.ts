...
export class RenderService {
  ...
  private ffmpegFlashBlackConcat(
    ffmpegPath: string,
    clips: Array<{ key: string; src: string; duration: number }>,
    outputPath: string,
    qualPreset: QualityPreset,
  ...
  async render(request: RenderRequest): Promise<RenderResult> {
    ...
    if (['trim-concat', 'xfade-concat', 'trim-xfade-concat', 'zoom-dissolve-concat', 'flash-black-concat'].includes(request.templateId)) {
    ...
    try {
      ...
      // ── flash-black-concat：前段尾部淡黑、后段头部从黑淡入 ──
